...
static vector<string> weaponPreviews{ "guns/ak47_preview.png","guns/Sniper_preview.png", "guns/heavyGun_preview.png", "guns/beretta_preview.png" };
static vector<string> weapons{ "guns/ak47.png","guns/Sniper.png", "guns/heavyGun.png","guns/beretta.png" };
...
static Table table((float)0, (float)-2.5, (float)0, (float)0.18, (float)0.5, (float)2);
static vector<Ammobox> ammoboxes{ Ammobox(-12, -0.25, 0, 0.5, 0.5, 135),
									Ammobox(12, -0.25, 0, 0.5, 0.5, 45) };

static vector<Bullet> bullets{ Bullet(-10, -0.9, 2, 0.08, 0.08, 90, -45),
								Bullet(-10.5, -0.8, 2, 0.08, 0.08, 0, 0),
								Bullet(-9.5, -0.9, 1.5, 0.08, 0.08, 90, -25),
								Bullet(-10, -0.9, 1.7, 0.08, 0.08, 90, -85),
								Bullet(+10, -0.9, 1.7, 0.08, 0.08, 0, 0),
								Bullet(+7, -0.9, 0.3, 0.08, 0.08, 90, 145) };

static vector<Target> targets{ Target(0, -2.5, -5, 0.5, 0.5, 90),
								Target(-10, -2.5, -10, 0.5, 0.5, 90),
								Target(+10, -2.5, -7, 0.5, 0.5, 90), 
								Target(-5, -2.5, -8, 0.5, 0.5, 90), 
								Target(+5, -2.5, -9, 0.5, 0.5, 90)};
...
ISound* inGameSound;
static vector<const char*> gunSounds{ "Audio/ak47_sound.wav", "Audio/ak47_sound.wav", "Audio/beretta_sound.wav","Audio/beretta_sound.wav"};
//=======================//
...
static int selectedWeapon = 0; // SELECTED WEAPON -> 0: ak47, 1: heavyGun, 2: beretta 3:sniper
...
void setup() {
	...
	menuTextureIds = textureAssets.loadTexture(vector<string>{"menu_background.jpg",
													...
													"logo_large.png",
													"guns/ak47_preview.png","guns/Sniper_preview.png", "guns/heavyGun_preview.png", "guns/beretta_preview.png",
													"weathers/sun_preview.png", "weathers/cloudy_preview.png"}, true, GL_LINEAR);
	//Loading game textures
	
	inGameTextureIds = textureAssets.loadTexture(vector<string>{"guns/ak47.png","guns/Sniper.png", "guns/heavyGun.png","guns/beretta.png",
													"game_interface.png",
													"environment/floor.jpg", "environment/sandbag.png","environment/wall.png", "environment/roof.jpg", "environment/holes.png",
													"environment/floornight.jpg", "environment/sandbagnight.png","environment/wallnight.png", "environment/roofnight.jpg",
													"environment/wall1.png","environment/wallnight1.png",
													"environment/floor1.jpg", "environment/floornight1.jpg","environment/roof1.jpg", "environment/roofnight1.jpg",
...
void resize(int w, int h) {
	...
	*/
	...
	window_width = w;
	window_height = h;
	glViewport(0, 0, w, h);//The viewport pixel 0,0 of the window has the same height and width as the window itself.
	glMatrixMode(GL_PROJECTION);//LET'S LOAD THE PROJECTION MATRIX
	glLoadIdentity();
	gluPerspective(72, ((GLfloat)w / h), 1.0, 150.0);
	glMatrixMode(GL_MODELVIEW); //LET'S LOAD THE MODELING MATRIX
	glLoadIdentity();
}
//=======================//
...
void managePicking() {
	if (picking)//if we are in picking mode
	{
		...
		GLuint color = GetRGB(mouseClickX, mouseClickY); //get the color of the pixel clicked with the left mouse button
		cout << "Color picked " << "0x" << color << ";" << endl;

		switch (color) //based on the color, we will perform different operations that will implement the functions pause, exit, restart, and resume
		{
		case COD_START:
			menu = false;
			inGame = true;
			...
			engine->stopAllSounds();
			inGameSound = engine->play2D("Audio/inGame_soundtrack.wav", true, false, true);
			inGameSound->setVolume(0.5);
			startTime = clock();
			break;
		case COD_DIFFICULTY:
			engine->play2D("Audio/pressed_button_sound.wav", false);
			difficulty = difficulty > 1 ? 0 : difficulty+1;
			cout << "Difficulty is = " << difficulty <<"\n";
			break;
		case COD_EXIT:
			exit(0);
			cout << "Clicked button is EXIT\n";
			break;
...
void drawScene(void) {
	...
	if (menu) {
		...
		//Difficulty button
		vector<const char*> difficulties = {"DIFFICULTY: EASY", "DIFFICULTY: MEDIUM", "DIFFICULTY: HARD"};
		...
		cameraMovement();
			drawEnvironment(X_MAP_LIMIT_RIGHT, X_MAP_LIMIT_LEFT,
				...
				selectedWeather, weathers, level,
		...
		drawGameInterface(score, level, difficulty, 
						...
						ammo, selectedWeapon, selectedWeather, weapons,
						textureAssets, inGameTextureIds);
...
void keyboard_special(int key, int x, int y) {
	...
	if (scoreboard && !hasEnteredName) {
		...
		case GLUT_KEY_F1:
			...
			table.setCoordinataZ(0.0);
			ammoboxes.at(0).setCoordinataZ(0.0);
			ammoboxes.at(1).setCoordinataZ(0.0);
			bullets.at(0).setCoordinataZ(0.0 + 1);
			bullets.at(1).setCoordinataZ(0.0 + 1);
			bullets.at(2).setCoordinataZ(0.0 + 1.5);
			bullets.at(3).setCoordinataZ(0.0 + 1.3);
			bullets.at(4).setCoordinataZ(0.0 + 1.3);
			bullets.at(5).setCoordinataZ(0.0 + 1.7);

			//restore targets lifes
			for (int i = 0; i < targetsColors.size(); ++i)
				targetsLife.at(i) = 2;

			engine->play2D("Audio/menu_theme.wav", true);
			break;
...
{	
	...
	glutCreateWindow("FIRING RANGE - 3D");
